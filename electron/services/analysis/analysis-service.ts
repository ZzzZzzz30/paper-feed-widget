...
function findSessionWithMessages(articleId: number): { id: number } | undefined {
  const db = getDb()
  return db.prepare(`
    SELECT s.id
    FROM analysis_sessions s
    WHERE s.article_id=?
      AND EXISTS (SELECT 1 FROM analysis_messages m WHERE m.session_id=s.id)
    ORDER BY s.id DESC
    LIMIT 1
  `).get(articleId) as { id: number } | undefined
}

function getLatestSession(articleId: number): { id: number } | undefined {
  return getDb().prepare('SELECT id FROM analysis_sessions WHERE article_id=? ORDER BY id DESC LIMIT 1').get(articleId) as { id: number } | undefined
}

function createSession(articleId: number): { id: number } {
  const db = getDb()
...
function getOrCreateWritableSession(articleId: number, sessionId?: number | null): { id: number } {
  if (sessionId && sessionId > 0) return { id: sessionId }
  return findSessionWithMessages(articleId) || getLatestSession(articleId) || createSession(articleId)
}

export async function quickSummary(articleId: number, sessionId?: number | null): Promise<{ sessionId: number; reply: string }> {
  const db = getDb()
  const session = getOrCreateWritableSession(articleId, sessionId)
  ...
  if (cached?.summary_cn) {
    db.prepare("INSERT INTO analysis_messages(session_id,role,content,created_at) VALUES(?,?,?,datetime('now'))").run(session.id, 'assistant', cached.summary_cn)
    return { sessionId: session.id, reply: cached.summary_cn }
  }
  ...
  if (!a) return { sessionId: session.id, reply: '文章数据不完整，无法分析' }
  if (!a.abstract && !a.abstract_cn) return { sessionId: session.id, reply: '当前文章摘要尚未缓存完成，无法生成概况。请稍后再试。' }
...
中文摘要：${a.abstract_cn || '无'}`
  ...
  try {
    ...
    db.prepare("INSERT INTO analysis_messages(session_id,role,content,created_at) VALUES(?,?,?,datetime('now'))").run(session.id, 'assistant', summary)
    return { sessionId: session.id, reply: summary }
  } catch (e) {
...
export function getOrCreateSession(articleId: number): { sessionId: number; messages: Array<{ role: string; content: string }> } {
  const db = getDb()
  const session = findSessionWithMessages(articleId) || getLatestSession(articleId) || createSession(articleId)
...
export async function chat(articleId: number, sessionId: number | null, message: string): Promise<{ sessionId: number; reply: string }> {
  ...
  // 获取历史消息
  const history = db.prepare('SELECT role,content FROM analysis_messages WHERE session_id=? ORDER BY id ASC LIMIT 20').all(session.id) as Array<{role:string;content:string}>
  ...
  // 保存用户消息
  db.prepare("INSERT INTO analysis_messages(session_id,role,content,created_at) VALUES(?,?,?,datetime('now'))").run(session.id, 'user', message)
  ...
  // 保存 AI 回复
  db.prepare("INSERT INTO analysis_messages(session_id,role,content,created_at) VALUES(?,?,?,datetime('now'))").run(session.id, 'assistant', reply)
